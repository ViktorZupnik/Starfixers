...
def SmaandE(vm, ha):
    sma = 1/(2/(ha*1000+6371000)-vm**2/mu)                          #calculate new semi major axis of spacecraft
    rp = sma*2-ha*1000-6371000
    e = (ha*1000+6371000-rp)/(ha*1000+rp+6371000)
...
#for 3 debris 
md = [250,300, 350, 400]  # Mass of debris in kg
h = [550, 580, 600, 630] # Altitude of debris in km
phi = np.array([50, 30, 120, 256])# Phase angle of debris in degrees
minimum_Mi = 10000000
min_mirdv = 1
...
for k in range(len(indicess)):
    ...
    time_tot = 0
    Sro = -5
    Vros = np.linspace(0, 8.5, 100)   # Wider range for Vro
    ...
    perigees = []
    apogees = []
    for zxc in range(int(period(debris_array[indicess[k][0],0], Vma)/60)):
        perigees.append(SmaandE(Vma, debris_array[indicess[k][0],0])[2])   # Store perigee heights of the spacecraft after each rendezvous
        apogees.append(debris_array[indicess[k][0],0])
    D_Vtot = np.abs(Vro)
    ...
    for j in range(len(debris_array)):   #10 debris 
        ...
        while D_Vbdtot <= dvbtot(debris_array[i, 1], debris_array[i,0]):                                                   #stop the while loop when delta V applied to debris is enough to deorbit
            ...
            for zxc in range(int(period(debris_array[i,0], Vma)/60)):
                perigees.append(SmaandE(Vma, debris_array[i,0])[2])      
                apogees.append(debris_array[i,0])
                                   #update our mass after momentum transfer   
        ...
        if j < len(debris_array)-1:  
            ...
            while phi_copy[i_next] > np.abs(catchup_phi(a2, debris_array[i_next ,0])):
                ...
                for zxc in range(int(period(debris_array[i_next,0], Vma)/60)):
                    perigees.append(SmaandE(Vma, debris_array[i_next,0])[2]) 
                    apogees.append(debris_array[i_next,0])
            Vc = np.sqrt(mu/((debris_array[i_next ,0]+6371)*1000))  #velocity of debris in circular orbit
            max_delta_phi = (period(debris_array[i_next ,0], Vc) - period(debris_array[i_next ,0], Vc-8))/(period(debris_array[i_next ,0], Vc))*360  #(0.7)maximum phase angle change for next rendezvous
            N = 1
        ...
        print(Mi, dry_mass) 
        if time_tot >= 7257600 and not_optimal==True:
            if Mi < minimum_Mi:
                minimum_Mi = Mi
                sequence = indicess[k]
    ...
    if Mi < minimum_Mi and time_tot < 7257600:    #2.8 months in seconds
        ...
        min_mirdv = bs
        not_optimal = False
    perigeess.append(perigees)
    apogeess.append(apogees)

num_orbits = len(perigeess)
rows, cols = 6, 2  # 4 rows x 2 columns = 8 subplots per figure
plots_per_fig = rows * cols

# Loop over two figures
for fig_idx in range(2):
    fig, axes = plt.subplots(rows, cols, figsize=(14, 12), sharex=False)
    axes = axes.flatten()

    for i in range(plots_per_fig):
        global_idx = fig_idx * plots_per_fig + i
        perigees = perigeess[global_idx]
        apogees = apogeess[global_idx]
        time_days = np.arange(len(perigees)) / 60 / 24  # assuming 1-minute steps
        ax = axes[i]
        ax.plot(time_days, perigees, label='Perigees', marker='o')
        ax.plot(time_days, apogees, label='Apogees', marker='x')
