...
def SmaandE(vm):
    ...
    e = (600000+6371000-rp)/(600000+rp+6371000)
    hp = (rp-6371000)/1000  # Perigee height in km
    return sma, e, hp

bs = 0 
perigees = []
phi = np.linspace(0, 360, 10)
for zxc in range(int(period(600, Vm)/60)):
    perigees.append(SmaandE(Vm)[2])   # Store perigee heights of the spacecraft after each rendezvous
time = 0
for i in range(10):   #10 debris 
    ...
    while D_Vbdtot <= 60.58:                                                   #stop the while loop when delta V applied to debris is enough to deorbit
        ...
        for zxc in range(int(period(600, Vm)/60)):
            perigees.append(SmaandE(Vm)[2])   # Store perigee heights of the spacecraft after each rendezvous  # Store perigee height after each rendezvous
        #check if it was the last burn 
        if D_Vbdtot >= 60.58:
           print('last burn')
           time += period(600, Vm)/2
           break 
        time += period(600, Vm)                                     
        ...
        time += period(600, Vm)             
        for zxc in range(int(period(600, Vm)/60)):
            perigees.append(SmaandE(Vm)[2])                             #update mass
        ...
        D_Vtot = D_Vtot + D_Vm + D_Vbm + np.abs(D_V_corr2)  # Store perigee heights of the spacecraft after each rendezvous
    bs += b
    ...
    if i < 9:        
        time += period(600, Vm)/2
        a1 = 1/(2/(600*1000+6371000)-Vm**2/mu) 
        for w in range(i+1,10):   
            phi[w] = (phi[w] + phi_change(600,time))%360
        while phi[i+1] > np.abs(catchup_phi(a1, 600)):
            delta_phi = catchup_phi(a1, 600)  #calculate phase angle change for next rendezvous
            phi[i+1]= (phi[i+1] + delta_phi)  #update phase angle for next rendezvous
            time += period(600, Vm)  #update time after phase angle change
            for zxc in range(int(period(600, Vm)/60)):
                perigees.append(SmaandE(Vm)[2]) 
        Vc = np.sqrt(mu/((600+6371)*1000))  #velocity of debris in circular orbit
        max_delta_phi = (period(600, Vc) - period(600, Vc-5))/(period(600, Vc))*360  #(0.7)maximum phase angle change for next rendezvous
        N = 1
        while np.abs(phi[i+1]/N) > np.abs(max_delta_phi):
            N += 1
        delta_phi = -phi[i+1]/N  #calculate phase angle change for next rendezvous
        T_adr = delta_phi*period(600, Vc)/360+ period(600, Vc)
        a_adr = ((T_adr/(2*np.pi))**(2/3))*mu**(1/3)  #semi major axis of our spacecraft after phase angle change
        D_V_trans = np.sqrt(mu*(2/((600+6371)*1000)-1/a_adr))-Vm  #update Vm for next rendezvous
        D_Vtot += np.abs(D_V_trans)
        Vm += D_V_trans
        M = M/(np.exp(np.abs(D_V_trans)/(Isp*g0)))  #update mass after transfer velocity
        time += N*period(600, Vm)  #update time after transfer velocity
        for qwe in range(N):
            for zxc in range(int(period(600, Vm)/60)):
                perigees.append(SmaandE(Vm)[2]) 
        D_V_corr = np.sqrt(mu/((600 +6371)*1000)) -Vm - Vro
        Vm += D_V_corr              #add transfer velocity to rdv with new debris 
        D_Vtot = D_Vtot + np.abs(D_V_corr)
        M = M/(np.exp(np.abs(D_V_corr)/(Isp*g0)))  
    else:
        sma, e, hp = SmaandE(Vm)  #calculate sma and eccentricity of spacecraft orbit
        print(f'Velocity of spacecraft last rendezvous: {Vm} m/s, sma: {sma}, e: {e}')  #print velocity and sma of spacecraft after rendezvous
...
apogees = [600]*len(perigees)
perigees_ordered = sorted_arr = np.sort(perigees)[::-1]
#Plot in the same plot apogees and perigees against their indices
# plt.figure(figsize=(10, 6))
# plt.plot(np.arange(len(perigees)) / 60/24, perigees, label='Perigees', marker='o')
# plt.plot(np.arange(len(apogees)) / 60/24, apogees, label='Apogees', marker='x')
# plt.xlabel('Time (days)')
# plt.ylabel('Height (km)')
# plt.title('Perigees and Apogees during Operation')
# plt.legend()
# plt.grid()
# plt.show()

plt.figure(figsize=(10, 6))
plt.plot(np.arange(len(perigees)) / 60/24, perigees_ordered, label='Perigees', marker='o')
plt.plot(np.arange(len(apogees)) / 60/24, apogees, label='Apogees', marker='x')
plt.xlabel('Time (days)')
plt.ylabel('Height (km)')
plt.title('Ordered Perigees and Apogees during Operation')
plt.legend()
plt.grid()
plt.show()
print (time/60/60/24)